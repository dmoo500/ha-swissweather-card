...
export function renderDailyForecastDiagram(
  ...
  const hours = hourlyForecast.slice(0, days.length * 24);
      ...
      dayGroups.push(svg`
        <g>
          ...
          <foreignObject x="${x - iconSize / 2}" y="${iconY}" width="${iconSize}" height="${iconSize}">
              ${getWeatherIcon(days[d].condition || '', config.enable_animate_weather_icons ? 'animated' : 'mdiAsSVG', iconSize + 'px', true)}
          </foreignObject>
          <!-- Min/Max temp -->
