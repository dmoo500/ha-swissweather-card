    ...
    for (; (r = se.nextNode()) !== null && l.length < o; ) {
      if (r.nodeType === 1) {
        if (r.hasAttributes()) for (const u of r.getAttributeNames()) if (u.endsWith(xi)) {
          const p = d[a++], g = r.getAttribute(u).split(q), y = /([.?@])?(.*)/.exec(p);
...
let le = class extends P {
  ...
  render() {
    ...
    d > 0 && (d = 0);
    const p = 13, g = 64, y = 20, v = 18, F = 8, ve = v + p + F + g + F + 2, Re = 32, Oe = 60, W = ve + Re, E = W + Oe, V = u - d || 1, Z = e.map((f) => typeof f.precipitation == "number" ? f.precipitation : 0), Ot = e.map(
...
let _t = class Bi {
  constructor({ strings: e, _$litType$: t }, n) {
    ...
    this.parts = [];
    let s = 0, a = 0;
    const o = e.length - 1, l = this.parts, [c, d] = ln(e, t);
...
let cn = class {
  ...
  u(e) {
    ...
    for (; c !== void 0; ) {
      if (o === c.index) {
        let d;
        c.type === 2 ? d = new Gi(a, a.nextSibling, this, e) : c.type === 1 ? d = new c.ctor(a, c.name, c.strings, this, e) : c.type === 6 && (d = new fn(a, this, e)), this._$AV.push(d), c = r[++l];
      }
...
`, 1)[0], Z;
            ...
            if (Z.search(this.rules.other.nonSpaceChar) >= y || !p.trim()) d += `
` + Z.slice(y);
            else {
...
const T = (i, e) => i ? (e || (e = "24px"), h`<ha-icon
  ...
  const r = {
    ...
    "clear-night": h`<img src="${ns}" style="font-size:${t}" />`,
    cloudy: h`<img src="${ss}" style="font-size:${t}" />`,
    fog: h`<img src="${ls}" style="font-size:${t}" />`,
    hail: h`<img src="${cs}" style="font-size:${t}" />`,
    lightning: h`<img src="${ds}" style="font-size:${t}" />`,
    "lightning-rainy": h`<img src="${hs}" style="font-size:${t}" />`,
    partlycloudy: h`<img
      src="${n ? as : os}"
      style="font-size:${t}"
    />`,
    pouring: h`<img src="${ms}" style="font-size:${t}" />`,
    rainy: h`<img src="${ps}" style="font-size:${t}" />`,
    snowy: h`<img src="${us}" style="font-size:${t}" />`,
    "snowy-rainy": h`<img src="${fs}" style="font-size:${t}" />`,
    sunny: h`<img src="${ut}" style="font-size:${t}" />`,
    windy: h`<img src="${mi}" style="font-size:${t}" />`,
    "windy-variant": h`<img src="${mi}" style="font-size:${t}" />`,
    exceptional: h`<img src="${gs}" style="font-size:${t}" />`
  };
