 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: Zi, defineProperty: qi, getOwnPropertyDescriptor: Qi, getOwnPropertyNames: Yi, getOwnPropertySymbols: Ki, getPrototypeOf: Xi } = Object, Ge = globalThis, Rt = Ge.trustedTypes, Ji = Rt ? Rt.emptyScript : "", en = Ge.reactiveElementPolyfillSupport, ge = (i, e) => i, Re = { toAttribute(i, e) {
  switch (e) {
    case Boolean:
      i = i ? Ji : null;
      break;
...
ee.elementStyles = [], ee.shadowRootOptions = { mode: "open" }, ee[ge("elementProperties")] = /* @__PURE__ */ new Map(), ee[ge("finalized")] = /* @__PURE__ */ new Map(), en?.({ ReactiveElement: ee }), (Ge.reactiveElementVersions ??= []).push("2.1.1");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const ht = globalThis, Pe = ht.trustedTypes, Wt = Pe ? Pe.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, ai = "$lit$", I = `lit$${Math.random().toFixed(9).slice(2)}$`, oi = "?" + I, tn = `<${oi}>`, Q = document, we = () => Q.createComment(""), ve = (i) => i === null || typeof i != "object" && typeof i != "function", dt = Array.isArray, nn = (i) => dt(i) || typeof i?.[Symbol.iterator] == "function", qe = `[ 	
...
const sn = (i, e) => {
  ...
  for (let o = 0; o < t; o++) {
    ...
    r += a === de ? l + tn : d >= 0 ? (s.push(c), l.slice(0, d) + ai + l.slice(d) + I + u) : l + I + (d === -2 ? o : u);
  }
    ...
    for (; (n = Z.nextNode()) !== null && l.length < o; ) {
      if (n.nodeType === 1) {
        if (n.hasAttributes()) for (const d of n.getAttributeNames()) if (d.endsWith(ai)) {
          ...
          l.push({ type: 1, index: r, name: y[2], strings: u, ctor: y[1] === "." ? an : y[1] === "?" ? on : y[1] === "@" ? ln : Ie }), n.removeAttribute(d);
 ...
 */
var Qe;
const We = window, se = We.trustedTypes, Gt = se ? se.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, it = "$lit$", B = `lit$${(Math.random() + "").slice(9)}$`, yi = "?" + B, xn = `<${yi}>`, Y = document, ze = () => Y.createComment(""), be = (i) => i === null || typeof i != "object" && typeof i != "function", wi = Array.isArray, bn = (i) => wi(i) || typeof i?.[Symbol.iterator] == "function", Ye = `[ 	
\f\r]`, pe = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, It = /-->/g, Bt = />/g, V = RegExp(`>|${Ye}(?:([^\\s"'>=/]+)(${Ye}*=${Ye}*(?:[^ 	
...
const _n = (i, e) => {
  ...
  for (let o = 0; o < t; o++) {
    const l = i[o];
    let c, h, d = -1, f = 0;
    for (; f < l.length && (a.lastIndex = f, h = a.exec(l), h !== null); ) f = a.lastIndex, a === pe ? h[1] === "!--" ? a = It : h[1] !== void 0 ? a = Bt : h[2] !== void 0 ? (vi.test(h[2]) && (n = RegExp("</" + h[2], "g")), a = V) : h[3] !== void 0 && (a = V) : a === V ? h[0] === ">" ? (a = n ?? pe, d = -1) : h[1] === void 0 ? d = -2 : (d = a.lastIndex - h[2].length, c = h[1], a = h[3] === void 0 ? V : h[3] === '"' ? jt : Ft) : a === jt || a === Ft ? a = V : a === It || a === Bt ? a = pe : (a = V, n = void 0);
...
`, i = i.substring(f.length + 1), l = !0), !l) {
          ...
          for (; i; ) {
            let O = i.split(`
`, 1)[0], G;
            if (f = O, this.options.pedantic ? (f = f.replace(this.rules.other.listReplaceNesting, "  "), G = f) : G = f.replace(this.rules.other.tabCharGlobal, "    "), ke.test(f) || R.test(f) || N.test(f) || ce.test(f) || $e.test(f)) break;
...
let F = class extends te {
  ...
  _renderDailyForecastDiagram() {
    ...
    h > 0 && (h = 0);
    const f = 13, u = 64, y = 20, b = 18, E = 8, ce = b + f + E + u + E + 2, $e = 32, ke = 60, R = ce + $e, N = R + ke, O = d - h || 1, G = e.map((g) => typeof g.precipitation == "number" ? g.precipitation : 0), $t = e.map(
